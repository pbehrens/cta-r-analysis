...
query <- "SELECT * , count(DISTINCT run_number) AS count FROM trips_trip GROUP BY id"
...
min.time <- as.POSIXlt("2012-10-30 00:39:55")
# min.time <- times(format(min.time, "%Y-%m-%d %H:%M:%S"))
max.time <- as.POSIXlt("2012-11-01 00:10:55")
# max.time <- times(format(max.time, "%Y-%m-%d %H:%M:%S"))
trips <- subset(trips, trips$prediction_generated > min.time)
trips <- subset(trips, trips$prediction_generated < max.time)
